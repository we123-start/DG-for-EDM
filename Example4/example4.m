...
dt=deltat;
%dt=final_t/Nt;
% uumin=0.1638;    %picture 1 ×
% vvmin=0.1563;
% uumin=0.0451;    %picture 2
% vvmin=0.0476;
% uumin=0.1778;    %picture 3 ×
% vvmin=0.0667;
uumin=0.0317;    %picture 4
vvmin=0.3016;
% uumin=0.2;    %example 5
% vvmin=0.3;
% uumin=0.4211;    %picture 5 ×
% vvmin=0.0058;
% f0=0.0423;    %picture 1 ×
% g0=0.1005;
% f0=0.1818;    %picture 2
% g0=0.1142;
% f0=0.0338;    %picture 3 and 5
% g0=0.2627;
f0=0.2051;    %picture 4
g0=0.0598;
% f0=0.5;    %example 5
    ...
    for j=1:J
        for i=0:p
            ...
            if j==1
                ...
                uminus=uumin;%left boundary conditions
                penalty=h\beta1*(uplus-uminus);
                ...
                vminus=vvmin;%left boundary conditions
                penalty=h\beta1*(vplus-vminus);
    ...
    for j=1:J
        if j==1
            ...
            while(q>1)
                ...
                c=u(q-1,kk,j)-uumin;%left boundary conditions
                d=minmod(aa,b,c);
                u(q,kk,j)=(2*q-3)\d;
                aa=(2*q-3)*v(q,kk,j);
                b=v(q-1,kk,j+1)-v(q-1,kk,j);
                %                 c=v(q-1,kk,j)-v(q-1,kk,J);%Periodic boundary conditions
                c=v(q-1,kk,j)-vvmin;%left boundary conditions
                d=minmod(aa,b,c);
                v(q,kk,j)=(2*q-3)\d;
                q=q-1;
            end
...
%                 sum_loc1_6=0;
%                 for k=0:p
%                     sum_loc1_1=sum_loc1_1+w(k+1).*(1.5*(1+sum(v(1:p1,kk-1,j).*P(1:p+1,k+1)))/(1-sum(u(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     sum_loc1_2=sum_loc1_2+w(k+1).*(-1.5*sum(u(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     % sum_loc1_3=sum_loc1_3+w(k+1).*(-1.5*sum(u(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1);
%                     sum_loc1_4=sum_loc1_4+w(k+1).*(3*(1-sum(u(1:p1,kk-1,j).*P(1:p+1,k+1)))/(1-sum(u(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     sum_loc1_5=sum_loc1_5+w(k+1).*(3*sum(v(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     % sum_loc1_6=sum_loc1_6+w(k+1).*(-3*sum(v(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1);
%                 end
%                 E1(i+1,m+1)=h/2*sum_loc1_1;
%                 F1(i+1,m+1)=h/2*sum_loc1_2;
...
%             if j==1
%                 uplus=sum(u(:,kk-1,j+1).*temp1(:));
%                 uuminus=sum(u(:,kk-1,j).*ones(p1,1));
%                 penalty=h\beta1*(uplus-uuminus);
%                 uxplus=u(2,kk-1,j+1)*2/h+u(3,kk-1,j+1)*2/h*3*(-1)+u(4,kk-1,j+1)*2/h*6;
%                 uxminus=u(2,kk-1,j)*2/h+u(3,kk-1,j)*2/h*3+u(4,kk-1,j)*2/h*6;
%                 uxxplus=u(3,kk-1,j+1)*(2/h)^2*3+u(4,kk-1,j+1)*(2/h)^2*15*(-1);
%                 uxxminus=u(3,kk-1,j)*(2/h)^2*3+u(4,kk-1,j)*(2/h)^2*15;
%                 A1r=Dapp*(penalty+1/2*(uxplus+uxminus)+beta2*h*(uxxplus-uxxminus))-1/2*(uplus+uuminus)-Dapp*(1/2*(uplus+uuminus)-uuminus)*wxminus(i+1);
%                 uplus=sum(u(:,kk-1,j).*temp1(:));
%                 uuminus=uumin;%left boundary conditions
%                 penalty=h\beta1*(uplus-uuminus);
%                 uxplus=u(2,kk-1,j)*2/h+u(3,kk-1,j)*2/h*3*(-1)+u(4,kk-1,j)*2/h*6;
%                 uxminus=0;
%                 uxxplus=u(3,kk-1,j)*(2/h)^2*3+u(4,kk-1,j)*(2/h)^2*15*(-1);
%                 uxxminus=0;
%                 A1l=(Dapp*(penalty+1/2*(uxplus+uxminus)+beta2*h*(uxxplus-uxxminus))-1/2*(uplus+uuminus))*(-1).^i-Dapp*(1/2*(uplus+uuminus)-uplus)*wxplus(i+1);
%                 vplus=sum(v(:,kk-1,j+1).*temp1(:));
%                 vvminus=sum(v(:,kk-1,j).*ones(p1,1));
%                 penalty=h\beta1*(vplus-vvminus);
%                 vxplus=v(2,kk-1,j+1)*2/h+v(3,kk-1,j+1)*2/h*3*(-1)+v(4,kk-1,j+1)*2/h*6;
%                 vxminus=v(2,kk-1,j)*2/h+v(3,kk-1,j)*2/h*3+v(4,kk-1,j)*2/h*6;
%                 vxxplus=v(3,kk-1,j+1)*(2/h)^2*3+v(4,kk-1,j+1)*(2/h)^2*15*(-1);
%                 vxxminus=v(3,kk-1,j)*(2/h)^2*3+v(4,kk-1,j)*(2/h)^2*15;
%                 A2r=Dapp*(penalty+1/2*(vxplus+vxminus)+beta2*h*(vxxplus-vxxminus))-1/2*(vplus+vvminus)-Dapp*(1/2*(vplus+vvminus)-vvminus)*wxminus(i+1);
%                 vplus=sum(v(:,kk-1,j).*temp1(:));
%                 vvminus=vvmin;%left boundary conditions
%                 penalty=h\beta1*(vplus-vvminus);
%                 vxplus=v(2,kk-1,j)*2/h+v(3,kk-1,j)*2/h*3*(-1)+v(4,kk-1,j)*2/h*6;
%                 vxminus=0;
%                 vxxplus=v(3,kk-1,j)*(2/h)^2*3+v(4,kk-1,j)*(2/h)^2*15*(-1);
%                 vxxminus=0;
%                 A2l=(Dapp*(penalty+1/2*(vxplus+vxminus)+beta2*h*(vxxplus-vxxminus))-1/2*(vplus+vvminus))*(-1).^i-Dapp*(1/2*(vplus+vvminus)-vplus)*wxplus(i+1);
%             elseif j==J
%                 uplus=sum(u(:,kk-1,j).*ones(p1,1));%right boundary conditions
%                 uuminus=sum(u(:,kk-1,j).*ones(p1,1));
%                 penalty=h\beta1*(uplus-uuminus);
...
%                 A1r=Dapp*(penalty+1/2*(uxplus+uxminus)+beta2*h*(uxxplus-uxxminus))-1/2*(uplus+uuminus)-Dapp*(1/2*(uplus+uuminus)-uuminus)*wxminus(i+1);
%                 uplus=sum(u(:,kk-1,j).*temp1(:));
%                 uuminus=sum(u(:,kk-1,j-1).*ones(p1,1));
%                 penalty=h\beta1*(uplus-uuminus);
%                 uxplus=u(2,kk-1,j)*2/h+u(3,kk-1,j)*2/h*3*(-1)+u(4,kk-1,j)*2/h*6;
%                 uxminus=u(2,kk-1,j-1)*2/h+u(3,kk-1,j-1)*2/h*3+u(4,kk-1,j-1)*2/h*6;
%                 uxxplus=u(3,kk-1,j)*(2/h)^2*3+u(4,kk-1,j)*(2/h)^2*15*(-1);
...
%                 A2r=Dapp*(penalty+1/2*(vxplus+vxminus)+beta2*h*(vxxplus-vxxminus))-1/2*(vplus+vvminus)-Dapp*(1/2*(vplus+vvminus)-vvminus)*wxminus(i+1);
%                 vplus=sum(v(:,kk-1,j).*temp1(:));
%                 vvminus=sum(v(:,kk-1,j-1).*ones(p1,1));
%                 penalty=h\beta1*(vplus-vvminus);
%                 vxplus=v(2,kk-1,j)*2/h+v(3,kk-1,j)*2/h*3*(-1)+v(4,kk-1,j)*2/h*6;
%                 vxminus=v(2,kk-1,j-1)*2/h+v(3,kk-1,j-1)*2/h*3+v(4,kk-1,j-1)*2/h*6;
%                 vxxplus=v(3,kk-1,j)*(2/h)^2*3+v(4,kk-1,j)*(2/h)^2*15*(-1);
...
%             A2(i+1)=A2r-A2l;
%         end
%         U1(1:2*p1,kk-1,j)=[u(1:p1,kk-1,j);v(1:p1,kk-1,j)];
%         E=[M+E1 F1;F2 M+E2];
%         F=[M+E1+dt*(D_x-Dapp*D_xx) F1;F2 M+E2+dt*(D_x-Dapp*D_xx)];
%         G=[dt*A1';dt*A2'];
%         U1(1:2*p1,kk-1,j)=E\(F*U1(1:2*p1,kk-1,j)+G);
...
%                 sum_loc1_6=0;
%                 for k=0:p
%                     sum_loc1_1=sum_loc1_1+w(k+1).*(1.5*(1+sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1)))/(1-sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     sum_loc1_2=sum_loc1_2+w(k+1).*(-1.5*sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     % sum_loc1_3=sum_loc1_3+w(k+1).*(-1.5*sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1);
%                     sum_loc1_4=sum_loc1_4+w(k+1).*(3*(1-sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1)))/(1-sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     sum_loc1_5=sum_loc1_5+w(k+1).*(3*sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     % sum_loc1_6=sum_loc1_6+w(k+1).*(-3*sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u1(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v1(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1);
%                 end
%                 E1(i+1,m+1)=h/2*sum_loc1_1;
%                 F1(i+1,m+1)=h/2*sum_loc1_2;
%                 % G1(i+1)=h/2*sum_loc1_3;
...
%                 A1r=Dapp*(penalty+1/2*(u1xplus+u1xminus)+beta2*h*(u1xxplus-u1xxminus))-1/2*(u1plus+u1minus)-Dapp*(1/2*(u1plus+u1minus)-u1minus)*wxminus(i+1);
%                 u1plus=sum(u1(:,kk-1,j).*temp1(:));
%                 u1minus=uumin;%left bou1ndary conditions
%                 penalty=h\beta1*(u1plus-u1minus);
%                 u1xplus=u1(2,kk-1,j)*2/h+u1(3,kk-1,j)*2/h*3*(-1)+u1(4,kk-1,j)*2/h*6;
%                 u1xminus=0;
%                 u1xxplus=u1(3,kk-1,j)*(2/h)^2*3+u1(4,kk-1,j)*(2/h)^2*15*(-1);
%                 u1xxminus=0;
%                 A1l=(Dapp*(penalty+1/2*(u1xplus+u1xminus)+beta2*h*(u1xxplus-u1xxminus))-1/2*(u1plus+u1minus))*(-1).^i-Dapp*(1/2*(u1plus+u1minus)-u1plus)*wxplus(i+1);
...
%                 A2r=Dapp*(penalty+1/2*(v1xplus+v1xminus)+beta2*h*(v1xxplus-v1xxminus))-1/2*(v1plus+v1minus)-Dapp*(1/2*(v1plus+v1minus)-v1minus)*wxminus(i+1);
%                 v1plus=sum(v1(:,kk-1,j).*temp1(:));
%                 v1minus=vvmin;%left bou1ndary conditions
%                 penalty=h\beta1*(v1plus-v1minus);
%                 v1xplus=v1(2,kk-1,j)*2/h+v1(3,kk-1,j)*2/h*3*(-1)+v1(4,kk-1,j)*2/h*6;
%                 v1xminus=0;
%                 v1xxplus=v1(3,kk-1,j)*(2/h)^2*3+v1(4,kk-1,j)*(2/h)^2*15*(-1);
%                 v1xxminus=0;
%                 A2l=(Dapp*(penalty+1/2*(v1xplus+v1xminus)+beta2*h*(v1xxplus-v1xxminus))-1/2*(v1plus+v1minus))*(-1).^i-Dapp*(1/2*(v1plus+v1minus)-v1plus)*wxplus(i+1);
%             elseif j==J
%                 u1plus=sum(u1(:,kk-1,j).*ones(p1,1));%right bou1ndary conditions
%                 u1minus=sum(u1(:,kk-1,j).*ones(p1,1));
%                 penalty=h\beta1*(u1plus-u1minus);
%                 u1xplus=u1(2,kk-1,j)*2/h+u1(3,kk-1,j)*2/h*3*(-1)+u1(4,kk-1,j)*2/h*6;
%                 u1xminus=u1(2,kk-1,j)*2/h+u1(3,kk-1,j)*2/h*3+u1(4,kk-1,j)*2/h*6;
...
%                 A1l=(Dapp*(penalty+1/2*(u1xplus+u1xminus)+beta2*h*(u1xxplus-u1xxminus))-1/2*(u1plus+u1minus))*(-1).^i-Dapp*(1/2*(u1plus+u1minus)-u1plus)*wxplus(i+1);
%                 v1plus=sum(v1(:,kk-1,j).*ones(p1,1));%right bou1ndary conditions
%                 v1minus=sum(v1(:,kk-1,j).*ones(p1,1));
%                 penalty=h\beta1*(v1plus-v1minus);
%                 v1xplus=v1(2,kk-1,j)*2/h+v1(3,kk-1,j)*2/h*3*(-1)+v1(4,kk-1,j)*2/h*6;
%                 v1xminus=v1(2,kk-1,j)*2/h+v1(3,kk-1,j)*2/h*3+v1(4,kk-1,j)*2/h*6;
...
%             A2(i+1)=A2r-A2l;
%         end
%         U2(1:2*p1,kk-1,j)=[u1(1:p1,kk-1,j);v1(1:p1,kk-1,j)];
%         E=[M+E1 F1;F2 M+E2];
%         F=[M+E1+dt*(D_x-Dapp*D_xx) F1;F2 M+E2+dt*(D_x-Dapp*D_xx)];
%         G=[dt*A1';dt*A2'];
%         U2(1:2*p1,kk-1,j)=E\(F*U2(1:2*p1,kk-1,j)+G);
...
%                 sum_loc1_6=0;
%                 for k=0:p
%                     sum_loc1_1=sum_loc1_1+w(k+1).*(1.5*(1+sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1)))/(1-sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     sum_loc1_2=sum_loc1_2+w(k+1).*(-1.5*sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     % sum_loc1_3=sum_loc1_3+w(k+1).*(-1.5*sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1);
%                     sum_loc1_4=sum_loc1_4+w(k+1).*(3*(1-sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1)))/(1-sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     sum_loc1_5=sum_loc1_5+w(k+1).*(3*sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1).*P(m+1,k+1);
%                     % sum_loc1_6=sum_loc1_6+w(k+1).*(-3*sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1))/(1-sum(u2(1:p1,kk-1,j).*P(1:p+1,k+1))+sum(v2(1:p1,kk-1,j).*P(1:p+1,k+1)))^2.)*P(i+1,k+1);
%                 end
%                 E1(i+1,m+1)=h/2*sum_loc1_1;
%                 F1(i+1,m+1)=h/2*sum_loc1_2;
%                 % G1(i+1)=h/2*sum_loc1_3;
...
%                 A1r=Dapp*(penalty+1/2*(u2xplus+u2xminus)+beta2*h*(u2xxplus-u2xxminus))-1/2*(u2plus+u2minus)-Dapp*(1/2*(u2plus+u2minus)-u2minus)*wxminus(i+1);
%                 u2plus=sum(u2(:,kk-1,j).*temp1(:));
%                 u2minus=uumin;%left bou2ndary conditions
%                 penalty=h\beta1*(u2plus-u2minus);
%                 u2xplus=u2(2,kk-1,j)*2/h+u2(3,kk-1,j)*2/h*3*(-1)+u2(4,kk-1,j)*2/h*6;
%                 u2xminus=0;
%                 u2xxplus=u2(3,kk-1,j)*(2/h)^2*3+u2(4,kk-1,j)*(2/h)^2*15*(-1);
%                 u2xxminus=0;
%                 A1l=(Dapp*(penalty+1/2*(u2xplus+u2xminus)+beta2*h*(u2xxplus-u2xxminus))-1/2*(u2plus+u2minus))*(-1).^i-Dapp*(1/2*(u2plus+u2minus)-u2plus)*wxplus(i+1);
...
%                 A2r=Dapp*(penalty+1/2*(v2xplus+v2xminus)+beta2*h*(v2xxplus-v2xxminus))-1/2*(v2plus+v2minus)-Dapp*(1/2*(v2plus+v2minus)-v2minus)*wxminus(i+1);
%                 v2plus=sum(v2(:,kk-1,j).*temp1(:));
%                 v2minus=vvmin;%left bou2ndary conditions
%                 penalty=h\beta1*(v2plus-v2minus);
%                 v2xplus=v2(2,kk-1,j)*2/h+v2(3,kk-1,j)*2/h*3*(-1)+v2(4,kk-1,j)*2/h*6;
%                 v2xminus=0;
%                 v2xxplus=v2(3,kk-1,j)*(2/h)^2*3+v2(4,kk-1,j)*(2/h)^2*15*(-1);
%                 v2xxminus=0;
%                 A2l=(Dapp*(penalty+1/2*(v2xplus+v2xminus)+beta2*h*(v2xxplus-v2xxminus))-1/2*(v2plus+v2minus))*(-1).^i-Dapp*(1/2*(v2plus+v2minus)-v2plus)*wxplus(i+1);
%             elseif j==J
%                 u2plus=sum(u2(:,kk-1,j).*ones(p1,1));%right bou2ndary conditions
%                 u2minus=sum(u2(:,kk-1,j).*ones(p1,1));
%                 penalty=h\beta1*(u2plus-u2minus);
%                 u2xplus=u2(2,kk-1,j)*2/h+u2(3,kk-1,j)*2/h*3*(-1)+u2(4,kk-1,j)*2/h*6;
%                 u2xminus=u2(2,kk-1,j)*2/h+u2(3,kk-1,j)*2/h*3+u2(4,kk-1,j)*2/h*6;
...
%                 A1l=(Dapp*(penalty+1/2*(u2xplus+u2xminus)+beta2*h*(u2xxplus-u2xxminus))-1/2*(u2plus+u2minus))*(-1).^i-Dapp*(1/2*(u2plus+u2minus)-u2plus)*wxplus(i+1);
%                 v2plus=sum(v2(:,kk-1,j).*ones(p1,1));%right bou2ndary conditions
%                 v2minus=sum(v2(:,kk-1,j).*ones(p1,1));
%                 penalty=h\beta1*(v2plus-v2minus);
%                 v2xplus=v2(2,kk-1,j)*2/h+v2(3,kk-1,j)*2/h*3*(-1)+v2(4,kk-1,j)*2/h*6;
%                 v2xminus=v2(2,kk-1,j)*2/h+v2(3,kk-1,j)*2/h*3+v2(4,kk-1,j)*2/h*6;
...
%             A2(i+1)=A2r-A2l;
%         end
%         U(1:2*p1,kk-1,j)=[u2(1:p1,kk-1,j);v2(1:p1,kk-1,j)];
%         E=[M+E1 F1;F2 M+E2];
%         F=[M+E1+dt*(D_x-Dapp*D_xx) F1;F2 M+E2+dt*(D_x-Dapp*D_xx)];
%         G=[dt*A1';dt*A2'];
%         U(1:2*p1,kk-1,j)=E\(F*U(1:2*p1,kk-1,j)+G);
%         u(1:p1,kk,j)=U(1:p1,kk-1,j);
%         v(1:p1,kk,j)=U(p1+1:2*p1,kk-1,j);
%     end
%     %%% the limiter function down
%     for j=1:J
%         if j==1
...
figure;
subplot(1,2,1);
h1 = surf(X, T, unum);            % 用第四个参数默认就是 Z 本身来着色
...
subplot(1,2,2);
h2 = surf(X, T, vnum);           % 假设 unum2 是 c2 的数值解
